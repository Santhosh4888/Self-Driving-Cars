...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            if dt == 0:
                dt = 0.01   # To avoid division by zero in the first step
            ...
            if acceleration > 0:
                throttle_output = (np.tanh(acceleration) + 1) / 2  # Normalize to [0, 1]
                if throttle_output < 0.1: # throttle_output = max(0.0, min(1.0, throttle_output))
                    throttle_output = 0.1
                elif throttle_output - self.vars.throttle_previous > 0.1:
            ...
            # Using stanley controller for lateral control
            k_e = 0.5  # Gain for cross-track error
            ...
            path_yaw = np.arctan2((waypoints[-1][1] - waypoints[0][1]), (waypoints[-1][0] - waypoints[0][0]))
            # path_yaw = np.arctan2(slope, 1.0)  # This was turning the vehicle only to the right (some error)
            ...
            steer_expect = yaw_diff_crosstrack + heading_error
            if steer_expect > self._pi:
                steer_expect -= 2 * self._pi
            if steer_expect < - self._pi:
                steer_expect += 2 * self._pi
